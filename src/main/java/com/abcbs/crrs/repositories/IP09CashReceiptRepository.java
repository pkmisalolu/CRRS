...
import com.abcbs.crrs.projections.ICRActivityView;
import com.abcbs.crrs.projections.ICRLocationView;
import com.abcbs.crrs.projections.ICROptActivityView;
...
public interface IP09CashReceiptRepository extends JpaRepository<P09CashReceipt, CashReceiptPK> {
	...
	//PATBNAMEEQ  when first name and last name doesn't contain generics
		@Query("SELECT c.crId.crRefundType AS crRefundType, c.crId.crCntrlDate AS crCntrlDate, c.crId.crCntrlNbr AS crCntrlNbr, c.crStatusText AS crStatusText, c.crCntrldAmt AS crCntrldAmt, c.crReceiptType AS crReceiptType "
				+ " FROM P09CashReceipt c " +
				"WHERE c.crId.crRefundType = :refType AND c.crPatientLname = :patLname AND c.crPatientFname = :patFname  " +
				"ORDER BY c.crId.crCntrlDate, c.crId.crCntrlNbr")
		public	List<ICRSuperView> findByRefTypeAndFullNameRange(@Param("refType") String refType, @Param("patLname") String patLname,@Param("patFname") String patFname) ;
	...
	@Query("SELECT c.crId.crRefundType AS crRefundType, c.crId.crCntrlDate AS crCntrlDate, c.crId.crCntrlNbr AS crCntrlNbr, c.crStatusText AS crStatusText, c.crCntrldAmt AS crCntrldAmt, c.crReceiptType AS crReceiptType "
			...
			"WHERE c.crId.crRefundType = :refType AND c.crPatientLname = :patLname AND c.crPatientFname = :patFname  " +
			"ORDER BY c.crId.crCntrlDate DESC, c.crId.crCntrlNbr")
	public	List<ICRSuperView> findByRefTypeAndFullNameRangeDesc(@Param("refType") String refType, @Param("patLname") String patLname,@Param("patFname") String patFname) ;
	...
	//CASHCUR cursor P09150
	@Query("SELECT c.crId.crRefundType AS crRefundType, c.crId.crCntrlDate AS crCntrlDate, c.crId.crCntrlNbr AS crCntrlNbr, " +
			"c.crClaimType AS crClaimType FROM P09CashReceipt c WHERE c.crId.crRefundType = :refType " +
			"AND c.crId.crCntrlDate = :cntrlDate AND c.crId.crCntrlNbr = :cntrlNbr")
	public	List<ICRActivityView> findCashClaimDetails(@Param("refType") String refType, @Param("cntrlDate") LocalDate cntrlDate, @Param("cntrlNbr") String cntrlNbr);


	@Query("SELECT c.crId.crRefundType AS crRefundType, c.crId.crCntrlDate AS crCntrlDate, c.crId.crCntrlNbr AS crCntrlNbr, " +
			"c.crEntryDate AS crEntryDate, c.crStatusDate AS crStatusDate, c.crStatusText AS crStatusText, c.crPendFinAct AS crPendFinAct, " +
			"c.crCntrldAmt AS crCntrldAmt, c.crReceiptBal AS crReceiptBal, c.crLetterDate AS crLetterDate, c.crLocationNbr AS crLocationNbr, " +
			"c.crLocationClerk AS crLocationClerk, c.crGlAcctNbr AS crGlAcctNbr, c.crCorp AS crCorp,c.crBankAcctNbr AS crBankAcctNbr,"
			+ " c.crPatientLname AS crPatientLname, c.crPatientFname AS crPatientFname, c.crMbrIdNbr AS crMbrIdNbr, c.crUserId AS crUserId FROM P09CashReceipt c " +
			"WHERE c.crId.crRefundType = :refType AND c.crId.crCntrlDate = :cntrlDate AND c.crId.crCntrlNbr = :cntrlNbr")
	public	List<ICROptActivityView> findCashReceiptDetails(@Param("refType") String refType, @Param("cntrlDate") LocalDate cntrlDate, @Param("cntrlNbr") String cntrlNbr);

	@Modifying
	@Transactional
	@Query("UPDATE P09CashReceipt c SET c.crStatusDate = :statusDate, c.crStatusText = :statusText, c.crPendFinAct = :pendFinAct, " +
			"c.crReceiptBal = :receiptBal WHERE c.crId.crRefundType = :refundType AND c.crId.crCntrlDate = :cntrlDate AND c.crId.crCntrlNbr = :cntrlNbr")
	public int updateCashReceiptStatus( @Param("statusDate") LocalDate statusDate, @Param("statusText") String statusText, @Param("pendFinAct") String pendFinAct,
			@Param("receiptBal") BigDecimal receiptBal, @Param("refundType") String refundType, @Param("cntrlDate") LocalDate cntrlDate,
			@Param("cntrlNbr") String cntrlNbr);
	
	@Query("SELECT c.crLocationNbr AS crLocationNbr, c.crLocationClerk AS crLocationClerk, c.crLocationDate AS crLocationDate FROM P09CashReceipt c " +
		       "WHERE c.crId.crRefundType = :refundType AND c.crId.crCntrlDate = :cntrlDate AND c.crId.crCntrlNbr = :cntrlNbr")
	public	List<ICRLocationView> findLocationInfoByControl(@Param("refundType") String refundType, @Param("cntrlDate") LocalDate cntrlDate, @Param("cntrlNbr") String cntrlNbr);

	@Modifying
	@Transactional
	@Query("UPDATE P09CashReceipt c SET c.crLocationNbr = :locationNbr, c.crLocationClerk = :locationClerk, c.crLocationDate = :locationDate " +
	       "WHERE c.crId.crRefundType = :refundType AND c.crId.crCntrlDate = :cntrlDate AND c.crId.crCntrlNbr = :cntrlNbr")
	public int updateLocationFields(@Param("locationNbr") String locationNbr, @Param("locationClerk") String locationClerk, @Param("locationDate") LocalDate locationDate,
	    @Param("refundType") String refundType, @Param("cntrlDate") LocalDate cntrlDate, @Param("cntrlNbr") String cntrlNbr);
