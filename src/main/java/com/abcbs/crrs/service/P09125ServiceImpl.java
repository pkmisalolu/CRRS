	...
	public List<ICRSuperView> fetch(P09125Binding binding) { 
		...
		if(inValidQuery) {
			...
			if(!binding.getCrProviderNbr().matches(".*[%_].*")) {
				...
				}//if provider number contains generics
			}else {
			...
			}//if member number contains generics
			else {
			...
			}//if tax id number contains generics
			else {
			...
			sortOrder = "CONTROL DATE / NBR";
			//if lastname doesn't contains any generics and firstname has entered
			if(!binding.getCrPatientLname().matches(".*[%_].*") && "1".equals(patFname)) {
				//if Firstname doesn't contain any generics
				if(!binding.getCrPatientFname().matches(".*[%_].*")){
					if("D".equals(binding.getSortDateOrder())) {
						list = repo.findByRefTypeAndFullNameRangeDesc(binding.getCrRefundType(), binding.getCrPatientLname(), binding.getCrPatientFname(), binding.getCrPatientFname());
					}else if("A".equals(binding.getSortDateOrder())) {
						list = repo.findByRefTypeAndFullNameRange(binding.getCrRefundType(), binding.getCrPatientLname(), binding.getCrPatientFname(), binding.getCrPatientFname());
					}//if Firstname contains generics
				}else {

				}
			}else {//if lastname doesn't contains generics and firstname has not entered
				if(!binding.getCrPatientLname().matches(".*[%_].*")) {
					if("D".equals(binding.getSortDateOrder())) {
					...
					}//if lastname contains generics and firstname has not entered
				}else {
			...
			}//else provider contains generics
			else {
			...
			if("CLOSED".equals(binding.getCrStatusText()))
				...
				}//else if provider contains generics
				else {
			...
			}//else if xref number contains generics
			else {
			...
			}//else if check number contains generics
			else {
			...
			if("CLOSED".equals(binding.getCrStatusText()))
				...
				}//else if member contains generics
				else {
